export const navMenu = [
  { name: "home", to: "/home", icon: "/images/home-icon.svg" },
  { name: "search", to: "/search", icon: "/images/search-icon.svg" },
  { name: "watchlist", to: "/watchlist", icon: "/images/watchlist-icon.svg" },
  { name: "original", to: "/original", icon: "/images/original-icon.svg" },
  { name: "movies", to: "/movies", icon: "/images/movie-icon.svg" },
  { name: "series", to: "/series", icon: "/images/series-icon.svg" },
  { name: "add", to: "/add", icon: "/images/series-icon.svg" },
];

export const slideImgs = [
  { name: "slide-img1", url: "/images/slider-badag.jpg" },
  { name: "slide-img2", url: "/images/slider-badging.jpg" },
  { name: "slide-img3", url: "/images/slider-scale.jpg" },
  { name: "slide-img4", url: "/images/slider-scales.jpg" },
  // {name: '', url: '/images/slider-bagag.jpg'}
];

export const viewerUrl = [
  {
    name: "disney",
    img: "/images/viewers-disney.png",
    video: "/videos/1564674844-disney.mp4",
  },
  {
    name: "pixar",
    img: "/images/viewers-pixar.png",
    video: "/videos/1564676714-pixar.mp4",
  },
  {
    name: "marvel",
    img: "/images/viewers-marvel.png",
    video: "/videos/1564676115-marvel.mp4",
  },
  {
    name: "starwars",
    img: "/images/viewers-starwars.png",
    video: "/videos/1608229455-star-wars.mp4",
  },
  {
    name: "national",
    img: "/images/viewers-national.png",
    video: "/videos/1564676296-national-geographic.mp4",
  },
];
